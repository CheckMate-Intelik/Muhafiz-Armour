...
export function VehicleCard({
  ...
  const rating =
    typeof vehicle.rating === 'number' && Number.isFinite(vehicle.rating) ? vehicle.rating : null;
  ...
  const statusClass =
    appearance === 'dark'
      ...
      : vehicle.isApproved
        ...
        : 'text-amber-600';
  const vehicleName =
    `${vehicle.manufacturer ?? ''} ${vehicle.generation ?? ''} ${vehicle.carModel ?? ''}`.trim() ||
    'Vehicle';
        ...
        <View className="relative overflow-hidden rounded-xl">
          ...
          {rating !== null ? (
            <View className="absolute left-2 top-2 flex-row items-center rounded-full bg-white px-2 py-1">
              ...
              <Text className="ml-1 text-[11px] font-extrabold text-gray-900">
                {rating.toFixed(1)}
              </Text>
            </View>
          ) : null}
