...
export function BookingDetailsBody({
  ...
  return (
    <>
      ...
      <View className="mt-4 rounded-3xl bg-gray-100 p-4" style={cardShadow}>
        <View className="mt-2">
          <Text className="text-lg font-bold text-gray-900">{vehicleName || '—'}</Text>
          <View className="mt-2 flex-row items-center gap-2">
            <Text className="text-sm font-bold text-gray-500">{vehicleType || '—'}</Text>
            <View className="flex-row items-center gap-1">
              <FontAwesome name="shield" size={16} color="#059669" />
              <Text className="text-md font-semibold text-gray-500">{vehicleArmour || '—'}</Text>
            </View>
          </View>
        </View>
        <View className="mt-3">
          <Text className="text-sm font-bold text-gray-400">Booking ID</Text>
          <Text className="mt-1 text-sm font-extrabold text-gray-900">{bookingId || '—'}</Text>
        </View>
      </View>

      <Text className="mt-6 text-xl font-bold text-gray-900">Trip info</Text>

      <View className="mt-5 flex-row items-center">
        <View className="relative h-11 w-11 items-center justify-center">
          <FontAwesome name="calendar-o" size={20} color="rgb(112, 112, 112)" />
          <View className="absolute bottom-0.5 right-0.5 rounded-sm bg-white">
            <FontAwesome name="clock-o" size={9} color="rgb(112, 112, 112)" />
          </View>
        </View>
        <View className="ml-1 flex-1">
          <Text className="text-md text-gray-600">
            Trip duration: <Text className="font-bold text-gray-900">{tripDurationText}</Text>
          </Text>
        </View>
      </View>
      ...
      <View className="mt-4 flex-row items-center">
        ...
        <View className="ml-1 flex-1">
          <Text className="text-md text-gray-600">
            Trip date: <Text className="font-bold text-gray-900">{tripStartDateText}</Text>
          </Text>
        </View>
      </View>
      ...
      <View className="mt-6 flex-row">
        <View className="items-center" style={{ width: 36 }}>
          <View className="h-9 w-9 items-center justify-center rounded-full bg-emerald-100">
            <FontAwesome name="map-marker" size={18} color="#059669" />
          </View>
          <View
            style={{
              alignSelf: 'center',
              ...
              marginVertical: 4,
              borderLeftWidth: 2,
              borderLeftColor: 'rgb(112, 112, 112)',
              borderStyle: 'dashed',
            }}
          />
          <View className="h-9 w-9 items-center justify-center rounded-full bg-red-100">
            <FontAwesome name="map-marker" size={18} color="#991B1B" />
          </View>
        </View>
        <View className="flex-1 pl-2">
          <Text className="text-md font-semibold text-gray-600">Pickup location</Text>
          <Text className="mt-0.5 text-lg font-bold text-gray-900" numberOfLines={4}>
            {pickupLocation || '—'}
          </Text>
          <Text className="mt-1 text-md font-medium text-gray-600">{pickupAtText}</Text>

          <View className="h-6" />

          <Text className="text-md font-semibold text-gray-600">Return location</Text>
          <Text className="mt-0.5 text-lg font-bold text-gray-900" numberOfLines={4}>
            {dropLocation || '—'}
          </Text>
          <Text className="mt-1 text-md font-medium text-gray-600">{returnAtText}</Text>
        </View>
      </View>
    </>
  );
